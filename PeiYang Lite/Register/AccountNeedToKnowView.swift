//
//  AccountNeedToKnowView.swift
//  PeiYang Lite
//
//  Created by phoenix Dai on 2021/3/1.
//
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false, content: {
            VStack(alignment: .leading, spacing: 10) {
                Text("微北洋用户需知").font(.title)
                Text("更新日期：2021年02月26日\n生效日期：2021年02月26日")
                Text("一．引言").font(.headline)
                Text("微北洋自推出以来连接全校师生，带来学习与生活的便捷。微北洋致力于为用户提供一个绿色、安全、健康、便捷的校园平台，为了实现这一目标，我们基于以下原则制定了《微北洋用户须知》，声明用户的权利与义务，对违规行为进行处理，维护用户及其他主体的合法权益。")
            ...
            VStack(alignment: .leading, spacing: 10) {
                Text("二．微北洋软件使用规范").font(.headline)
                Text("用户在使用微北洋软件的过程中不得实施影响其他用户体验、危及平台安全、损害他人权益的行为。一经发现，我们将根据违规程度对帐号采取相应的处理措施，并有权拒绝向违规帐号主体提供服务。如：限制与该主体相关帐号功能、封禁与该主体相关帐号等。")
                Text("1. 合理、善意注册使用微北洋").font(.headline)
                Text("你应当合理、善意注册并使用微北洋帐号，不得恶意注册或将天外天帐号用于非法或不正当用途。")
            ...
            VStack(alignment: .leading, spacing: 10) {
                Text("2. 数据获取、使用规范").font(.headline)

                Text("2.1 个人隐私").font(.headline)

                Text("2.1.1 当您登录办公网使用微北洋服务时，我们会收集您的姓名、学院信息、入学年份、专业、班级。收集这些信息是为了给您提供相应的服务。若您不提供这类信息，您无法正常使用我们提供的所有服务。")
                Text("2.1.2 在您使用微北洋校务专区服务期间，您可能会在发布页面中发布照片。在已知的需要上传照片的模块中，当您需要对照片进行选择上传时，我们会收集您通过相册权限主动上传的照片信息，即需要您授权我们读取您的相册权限。如果您拒绝授权仅会使您无法使用该功能，但不影响您正常使用微北洋其他功能。")
                Text("2.1.3 当您使用校务专区、失物招领功能时，我们会收集您上传的动图、照片、帖子、评论、点赞信息。您也可以随时删除这些信息。")
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text("2.2.1 微北洋保证不对外公开或向第三方透露用户个人隐私信息，或用户在使用服务时存储的非公开内容。")
                Text("2.2.2  请您注意，我们不会主动从第三方获取您的个人信息。")
...
//
                ...
                Text("微北洋用户明确了解并同意：")
                Text("微北洋app为用户所提供的课程、GPA、自习室等信息，均来自于天津大学教育信息管理中心、教务网站。我们尽可能保证为您提供最准确、及时、稳定的信息，一切准确信息以以上两个官方网站为准；若有错缺，请用户自行比对注意，由此造成的损失天外天工作室不承担任何责任。")
                Text("关于微北洋服务天外天工作室不提供任何种类的明示或暗示担保或条件，你对微北洋服务的使用行为必须自行承担相应风险。")

                Text("四、联系我们").font(.headline)
...
struct AccountNeedToKnowView_Previews: PreviewProvider {
