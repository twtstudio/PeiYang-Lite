...
class ChartData: ObservableObject, Identifiable {
    ...
    public func onlyPoints() -> [Double] {
        // to overlook this term if gpa is 0
//        return self.points.last?.1 == 0 && self.points.count > 1 ?
//            self.points.prefix(upTo: self.points.count - 2).map(\.1) :
//            self.points.map(\.1)
        self.points.map(\.1)
    }
}
...
extension UIColor {
    static func color(light: UIColor, dark: UIColor) -> UIColor {
        UIColor { traitCollection -> UIColor in
            ...
            case .dark:
                return dark
            @unknown default: // new feature introduced in Swift 5
...
extension Color {
    static func color(light: UIColor, dark: UIColor) -> Color {
        Color(UIColor.color(light: light, dark: dark))
    }
    
    static let background = color(light: .white, dark: .black)
    static let shadow = color(light: .init(white: 0, alpha: 0.2), dark: .init(white: 1, alpha: 0.2))
}
